...
// request must not pay a round trip per field, and media_url next to it makes
// the same promise from the same rows.
//
// The check is deliberately stricter than the one /thumb uses to pick an image
...
func captureHasStoredThumbnail(db *gorm.DB, item models.ArchiveItem) bool {
	...
	if err := db.Where("capture_id = ?", item.CaptureID).Find(&items).Error; err != nil {
		...
		// than failing a manifest over a preview.
