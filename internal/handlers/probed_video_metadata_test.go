...
// Instagram sometimes gives yt-dlp a complete MP4 while omitting duration
// from the info JSON. The final stored artifact is authoritative for that
// missing fact, but provider dimensions and codecs must remain untouched.
func TestNativeInstagramBackfillsMissingDurationFromStoredArtifact(t *testing.T) {
	...
	if metadata.Media.Width == nil || *metadata.Media.Width != 1080 || metadata.Media.Height == nil || *metadata.Media.Height != 1920 {
		t.Errorf("provider dimensions were overwritten by the probe: %+v", metadata.Media)
	}
	if metadata.Media.VideoCodec != "avc1.640028" || metadata.Media.AudioCodec != "mp4a.40.2" {
		t.Errorf("provider codecs were overwritten by the probe: %+v", metadata.Media)
	}
	if metadata.Media.FPS == nil || *metadata.Media.FPS != 30 || metadata.Media.BitrateKbps == nil || *metadata.Media.BitrateKbps != 2412.5 {
		t.Errorf("provider frame rate/bitrate were overwritten by the probe: %+v", metadata.Media)
	}
	if _, err := os.Stat(probedPath); err != nil {
		t.Fatalf("stored artifact was never probed: %v", err)
	}
	assertUnifiedVideoFacts(t, db, store, "igp01", 2.554195, 1080, 1920)
}
...
func (refusedVideoArchiver) Archive(context.Context, string, io.Writer, *gorm.DB, uint) (archivers.Result, error) {
	return archivers.Result{}, errors.New("facebook native extraction refused")
}
