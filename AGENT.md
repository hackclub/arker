...
- `GET /gallery/:shortid/raw` - Sanitized provider sidecars from the bundle, one record per asset, each with absolute `media_url`/`media_urls`. Every record also carries the cross-provider post fields `caption`, `user_posted`, `date_posted`, `photos_number`, `likes` and `num_comments`, **coerced to the types those names promise** — strings, an RFC 3339 UTC timestamp string, numbers — whichever shape the provider used: Instagram's Apify record publishes the caption as an object (`{id, pk, text, hashtags, …}`) and `taken_at` as epoch seconds, X and Facebook name the poster with a user object. A candidate that cannot be coerced is skipped in favour of the next key and finally Arker's own normalized metadata, and a structured provider value that coercion flattened is kept beside it as `<field>_details`
...
- `APIFY_API_TOKEN` - Enables the paid media fallback (see "Apify fallback"). When unset, `/data/secrets/apify-token` is read instead, so the token lives on the same redeploy-proof secrets mount as the cookie jar. Empty disables it entirely: no actor is started, and login-only sites with no cookie jar go back to producing no media item at all.
...
- Screenshot previews are lazily backfilled from their stored screenshot
  ...
  then uses the capped Apify poster resolver. Duplicate canonical URL/type
